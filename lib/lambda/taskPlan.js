...
const impl = {
  ...
  planPerformance: (timeNow, script, settings) => {
    const plan = []
    let updatedScript = impl.ensureScriptGenesis(script)

    // ## Duration ##
    const scriptDurationInSeconds = impl.scriptDurationInSeconds(updatedScript)
    ...
    if (scriptDurationInSeconds > settings.maxChunkDurationInSeconds) {
      const parts = impl.splitScriptByDurationInSecondsAndSchedule(timeNow, updatedScript, settings)
      updatedScript = parts.chunk
      plan.push(parts.remainder)
    }
    // ## Requests Per Second ##
    const scriptRequestsPerSecond = impl.scriptRequestsPerSecond(updatedScript)
    ...
    if (scriptRequestsPerSecond <= settings.maxChunkRequestsPerSecond) {
      ...
      const parts = impl.splitScriptByRequestsPerSecondAndSchedule(timeNow, updatedScript, settings)
  ...
  planSamples: (timeNow, script, settings) => {
    const updatedScript = impl.ensureScriptGenesis(script)
    ...
    return impl.splitScriptByFlow(updatedScript, settings)
  },
  ensureScriptGenesis: (script, timeNow) =>
    Object.assign(script, {
      _genesis: script._genesis || timeNow
    }),
  /**
...
module.exports.planSamples = impl.planSamples
module.exports.planPerformance = impl.planPerformance
