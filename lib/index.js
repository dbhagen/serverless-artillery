...
const impl = {
  ...
  getScriptText: (options) => {
    console.error(`getScriptText: ${JSON.stringify(options)}`)
  ...
  parseScript: (input) => {
    ...
    console.error(`parseScript: ${JSON.stringify(script)}`)
    return script
  },
  /**
...
module.exports = {
  ...
  invoke: options => impl.getScriptText(options)
    ...
    .then((input) => {
      const script = input
      console.error(`script: ${JSON.stringify(script)}`)
      if (modes.isAcceptanceScript(script)) {
        script.mode = modes.ACC
      } else if (modes.isMonitoringScript(script)) {
