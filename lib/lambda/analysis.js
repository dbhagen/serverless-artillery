...
const analysis = {
  ...
  getErrorBudget: (script, defaultErrorBudget) => {
    if (script && script.sampling && script.sampling.errorBudget) {
      ...
      return sampling.defaults.sampling.DefaultErrorBudget
    }
  },
  ...
  analyzeSamples: (script, report, defaultErrorBudget) => {
    ...
    const totalErrors = Object.keys(totals.errors).reduce((total, message) => totals.errors[message], 0)

    const errorBudget = analysis.getErrorBudget(script, defaultErrorBudget)
    // const sumErrors = subreport =>
    //   Object.keys(subreport.errors)
    //     .reduce((total, errorMessage) => (total + subreport.errors[errorMessage]), 0)
    ...
    //   const errorCount = subreport.errors ? sumErrors(subreport) : 0
    //   if (
    //     subreport &&
    //     subreport.errors &&
    //     errorCount > errorBudget
    //   ) {
    //     finalReport.errors += 1
    //   }
    // })
    if (finalReport.errors) {
    ...
    console.log(`Acceptance FINAL REPORT: ${JSON.stringify(finalReport, null, 2)}`)
  ...
  analyzeAcceptance: (timeNow, script, settings, results) =>
    analysis.analyzeSamples(script, results, sampling.defaults.acceptance.DefaultErrorBudget),
  ...
  analyzeMonitoring: (timeNow, script, settings, results) =>
    analysis.analyzeSamples(script, results, sampling.defaults.monitoring.DefaultErrorBudget),
  ...
  analyzePerformance: (timeNow, script, settings, results) => {
    console.error(timeNow, script, settings, results)
    if (results.length === 1) {
